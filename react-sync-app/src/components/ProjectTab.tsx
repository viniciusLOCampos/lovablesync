...
const ProjectTab = ({
  ...
  return (
    <div className={`space-y-6 transition-all duration-500 ease-out transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
      }`}>
      {/* Project Header */}
      <div className="relative z-10 bg-white/[0.02] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-6 shadow-2xl shadow-black/10">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            <div className={`p-4 rounded-2xl transition-all duration-300 ${config.auto_sync
          ...
          >
            ...
            {isSyncing ? (
              ...
              <>
                ...
                <span className="relative z-10">Sincronizar Agora</span>
                {config.auto_sync && (
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-slate-300">
                {progress.currentStep === 'listing' && 'Analisando arquivos...'}
                {progress.currentStep === 'deleting' && 'Excluindo arquivos destino...'}
                {progress.currentStep === 'copying' && 'Copiando arquivos origem...'}
                {progress.currentStep === 'completed' && 'Sincronização concluída!'}
                {progress.currentStep === 'error' && 'Erro na sincronização'}
              </span>
              <span className="text-sm text-slate-400">
                {progress.totalFiles > 0 && `${progress.filesProcessed}/${progress.totalFiles} arquivos`}
              </span>
            </div>
            ...
            {progress.totalFiles > 0 && (
              <div className="relative w-full bg-slate-800/50 rounded-full h-4 overflow-hidden shadow-inner border border-slate-700/50">
                <div
                  className={`h-4 rounded-full transition-all duration-500 ease-out relative ${progress.status === 'success' ? 'bg-gradient-to-r from-green-400 to-emerald-500' :
        ...
        <div className="border-b border-white/[0.08] bg-gradient-to-r from-slate-800/30 to-slate-900/30">
          <nav className="flex space-x-2 px-6">
            <button
              ...
              className={`group relative py-4 px-6 font-semibold text-sm transition-all duration-300 rounded-t-xl ${activeSection === 'overview'
            ...
            >
              <div className="flex items-center space-x-2">
                <BarChart3 className={`w-4 h-4 transition-transform duration-300 ${activeSection === 'overview' ? 'scale-110' : 'group-hover:scale-110'
                  }`} />
            ...
            <button
              ...
              className={`group relative py-4 px-6 font-semibold text-sm transition-all duration-300 rounded-t-xl ${activeSection === 'logs'
                ? 'text-blue-400 bg-slate-800/50 shadow-lg transform -translate-y-1 border-x border-t border-white/[0.08]'
                : 'text-slate-300 hover:text-blue-400 hover:bg-white/[0.05]'
                }`}
            >
              <div className="flex items-center space-x-2">
                <TrendingUp className={`w-4 h-4 transition-transform duration-300 ${activeSection === 'logs' ? 'scale-110' : 'group-hover:scale-110'
                  }`} />
                <span>Histórico</span>
                {projectLogs.length > 0 && (
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold transition-all duration-300 ${activeSection === 'logs'
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-left duration-300">
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm text-slate-400 bg-slate-800/50 px-4 py-2 rounded-full border border-slate-700/50">
                  {projectLogs.length} {projectLogs.length === 1 ? 'registro' : 'registros'}
                </div>
              </div>
